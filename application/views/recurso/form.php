    ...
    <div class="panel panel-default panel-border-color panel-border-color-primary">
      ...
      <div class="panel-body">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <input type="text" class="form-control" name="nome" value="<?= !empty($_POST) ? $this->input->post('nome') : (!empty($record) ? $record->rcs_nome : '') ?>" maxlength="250">
          </div>
        </div>
        <div class="form-group">
          <label class="col-sm-3 control-label">Telefone</label>
          <div class="col-sm-4">
            <input type="text" class="form-control" name="telefone" value="<?= !empty($_POST) ? $this->input->post('telefone') : (!empty($record) ? $record->rcs_telefone : '') ?>" maxlength="50">
          </div>
        </div>
        <div class="form-group">
          <label class="col-sm-3 control-label">Email</label>
          <div class="col-sm-6">
            <input type="email" class="form-control" name="email" value="<?= !empty($_POST) ? $this->input->post('email') : (!empty($record) ? $record->rcs_email : '') ?>" maxlength="100">
          </div>
        </div>
        <div class="form-group">
...
<script>
  window.addconhecimento = function(ev) {
    ...
    window.addbytxt(txtconhecimento);
  };
  window.addbytxt = function(txt) {
    ...
    cell.innerHTML = '' + txt + '<input type="hidden" name="conhecimento[]" value="' + txt + '" >';
  };
  <?php if(!empty($conhecimento)): ?>
  window.onload = function() {
    <?php
      foreach($conhecimento as $c) {
        echo "var txtconhecimento = '".$c->rc_conhecimento."';";
        echo "window.addbytxt(txtconhecimento);";
      }
    ?>
  };
  <?php endif; ?>
